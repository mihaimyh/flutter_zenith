...
void main() {
  ...
  group('Suite 1: Container Lifecycle & Memory Safety', () {
    test('Test 1.1: Multi-user logout teardown disposes nodes, runs onDispose '
        'hooks, and releases references', () {
      final containerA = ZenithContainer();
      var disposeHooksRun = 0;

      final userIdNode = containerA.getOrCreateNode<String>('userId', (ref) {
        ref.onDispose(() => disposeHooksRun++);
        return 'user-A';
      });

      final sessionNode = containerA.getOrCreateNode<int>('sessionTtl', (ref) {
        ref.onDispose(() => disposeHooksRun++);
        return 3600;
      });
    ...
    test('Test 1.3: Parent-child fallback does not leak or destroy state '
    ...
    test('Test 1.3b: invalidate() bubbles to the parent for missing keys '
        'without materializing a node in the child', () {
      final parent = ZenithContainer();
      final parentNode = parent.getOrCreateNode<int>('shared', (_) => 1);
      var parentNotifyCount = 0;
      parentNode.subscribe(_CallbackSubscriber((_) => parentNotifyCount++));
  ...
  group('Suite 2: Reactive Dependency Graph & Garbage Collection', () {
    test('Test 2.1: dead weak-referenced subscribers are purged from the node '
        'and debugSubscriberCount reflects reality', () async {
    ...
    test('Test 2.3a: invalidate() guards against synchronous re-entrant '
    ...
    test('Test 2.3b: circular invalidation between two nodes terminates '
    ...
    test('Test 2.3c: re-entrant set() with a converging value stops recursing '
  ...
  group('Suite 3: Async & Concurrent Execution', () {
    test('Test 3.1: out-of-order async resolution does not let a slow, '
        'earlier request overwrite a faster, newer one', () async {
    ...
    test('Test 3.2: async work started before disposal fails gracefully '
        'afterward instead of throwing an unhandled async exception', () async {
