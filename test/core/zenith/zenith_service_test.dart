...
void main() {
  group('ZenithService lifecycle', () {
    test(
      'registerService calls onStart and hooks onStop to scope disposal',
      () async {
    ...
    test(
      'ZenithPeriodicService executes task periodically and cancels timer on disposal',
      () async {
        final container = ZenithContainer();
        var executions = 0;
    ...
    test(
      'ZenithIsolateService spawns background worker isolate and kills it on disposal',
      () async {
        final container = ZenithContainer();
