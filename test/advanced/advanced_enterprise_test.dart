...
void main() {
  group('Phase 8: Multi-Isolate IPC, 3-Tier Hierarchy & Route Guards', () {
    test(
      'Test 8.1: 3-Tier Hierarchy allows Workspace swap without destroying UserScope',
      () {
    ...
    test(
      'Test 8.2: Two-Phase Scope Drain flushes uncommitted writes before termination',
      () async {
        final manager = ZenithScopeManager();
        final userScope = manager.getOrCreateScope('user_flusher');

        var flushCompleted = false;
        userScope.registerDrainable(
    ...
    test(
      'Test 8.3: Inter-Isolate IPC notifies foreground container to invalidate cache',
      () async {
        ...
        userScope.listenToInterIsolateInvalidations(
          portName,
          onInvalidate: (key) {
            if (key == 'items') {
    ...
    test(
      'Test 8.4: Declarative Route Guard denies navigation and returns redirect route',
      () {
