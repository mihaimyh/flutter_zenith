...
void main() {
  group('Phase 3: Multi-Tenant Partitioned Storage & Outbox', () {
    test(
      'Test 3.1: Disk storage keys are strictly isolated per tenant',
      () async {
        final mockDriver = InMemoryStorageDriver();
        final storageA = ZenithPartitionedStorage(
          driver: mockDriver,
          tenantId: 'user_a',
        );
        final storageB = ZenithPartitionedStorage(
          driver: mockDriver,
          tenantId: 'user_b',
        );
    ...
    test(
      'Test 3.2: Offline Outbox freezes inactive tenant queue on account switch',
      () async {
    ...
    test(
      'Test 3.3: Outbox continues flushing after individual mutation failure',
      () async {
        final outbox = ZenithOutboxEngine();
        outbox.setActiveTenant('user_a');
        ...
        expect(
          immediateAttempts,
          equals(0),
          reason: 'Must respect backoff window',
        );
