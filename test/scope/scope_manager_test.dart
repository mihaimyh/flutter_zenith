...
void main() {
  group('Phase 1: ZenithScopeManager & Scope Isolation', () {
    test(
      'Test 1.1: Scope Disposal completely isolates and frees tenant nodes',
      () {
        final manager = ZenithScopeManager();
        final userAScope = manager.getOrCreateScope('user_a');
        final node = userAScope.container.getOrCreate(
    ...
    test('Test 1.2: Captive Dependency Detection throws exception', () {
      ...
      expect(
        () => manager.appScope.register(rootServiceKey, () {
    ...
    test(
      ...
      () async {
        final manager = ZenithScopeManager();
        final userAScope = manager.getOrCreateScope('user_a');
        final stateNode = userAScope.container.getOrCreate(
          const ZenithKey<String>('data'),
          (_) => 'initial',
        );
        ...
        final userBNode = userBScope.container.getOrCreate(
          const ZenithKey<String>('data'),
          (_) => 'user_b_clean',
        );
